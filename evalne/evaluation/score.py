...
class Scoresheet:
    ...
    def get_pandas_df(self, metric='auroc', repeat=None):
        ...
        metric : basestring, optional
            ...
            'f_score', 'mcc', 'eval_time' or 'edge_embed_method'. Default is 'auroc'.
        repeat : int, optional
    ...
    def write_tabular(self, filename, metric='auroc'):
        ...
        metric : basestring, optional
            Can be one of 'tn', 'fp', 'fn', 'tp', 'auroc', 'precision', 'recall', 'fallout', 'miss', 'accuracy',
            'f_score', 'mcc' or 'eval_time'. Default is 'auroc'.
        """
        header = '\n\nEvaluation results ({}):\n-----------------------\n'.format(metric)
        f = open(filename, 'a')
        f.write(header)
...
class Results(object):
    ...
    def get_all(self, results='auto', precatk_vals=None):
        ...
        metric_names = ['tn', 'fp', 'fn', 'tp', 'auroc', 'precision', 'recall',
                        'fallout', 'miss', 'accuracy', 'f_score', 'mcc']
        metric_vals = [scores.tn, scores.fp, scores.fn, scores.tp, scores.auroc(), scores.precision(), scores.recall(),
                       scores.fallout(), scores.miss(), scores.accuracy(), scores.f_score(), scores.mcc()]
...
class Scores(object):
    ...
    def mcc(self):
        ...
        -------
        mcc : float
            A value indicating the MCC which is a correlation coefficient value between -1 and +1.
        """        
        return matthews_corrcoef(self.y_true, self.y_bin)
